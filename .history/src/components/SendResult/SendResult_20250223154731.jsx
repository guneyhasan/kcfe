...
const SendResult = ({ matchData, onBack, onSubmitResult, loading }) => {
    ...
    const handleSubmit = () => {
        ...
        onSubmitResult(selectedWinner);
    };

    return (
        <div className={styles.sendResultContainer}>
            <h2>Maç Sonucunu Bildir</h2>
            <div className={styles.playerSelection}>
                <div 
                    className={`${styles.player} ${selectedWinner === matchData.player1.id ? styles.selected : ''}`}
                    onClick={() => setSelectedWinner(matchData.player1.id)}
                >
                    <img src={matchData.player1.avatarUrl} alt={matchData.player1.username} />
                    <span>{matchData.player1.username}</span>
                </div>
                <div 
                    className={`${styles.player} ${selectedWinner === matchData.player2.id ? styles.selected : ''}`}
                    onClick={() => setSelectedWinner(matchData.player2.id)}
                >
                    <img src={matchData.player2.avatarUrl} alt={matchData.player2.username} />
                    <span>{matchData.player2.username}</span>
                </div>
            </div>
            <div className={styles.buttons}>
                <button onClick={onBack} disabled={loading}>Geri</button>
                <button 
