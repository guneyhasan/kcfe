...
const MeydanOkumalar = () => {
	...
	const handleSendChallengeRequest = async (challengeId) => {
		try {
			const requestData = {
				message: "Bu meydan okumaya katılmak istiyorum.", // Sabit mesaj veya form ile alınabilir
				platform: "Steam", // Platformu challenge'dan alabiliriz veya form ile
				platformUsername: "player456_steam" // Kullanıcıdan alabiliriz veya profilden çekebiliriz
			};

			const response = await challengeService.sendChallengeRequest(challengeId, requestData);
			
			if (response.success) {
				toast.success('Meydan okuma isteği başarıyla gönderildi!');
				// Listeyi yenile
				fetchChallenges();
			}
		} catch (error) {
			console.error('Meydan okuma isteği gönderilemedi:', error);
