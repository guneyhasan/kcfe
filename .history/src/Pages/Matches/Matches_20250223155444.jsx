...
const Matches = () => {
    ...
    const [matchStatus, setMatchStatus] = useState('all'); // 'all', 'pending', 'completed', 'disputed'
                ...
                if (response.success) {
                    ...
                    const allRequests = [
                        ...requests.pending.map(request => ({
                            id: request.id,
                            username: request.username || 'İsimsiz Kullanıcı',
                            avatarUrl: request.avatarUrl || 'default-avatar.png',
                            message: request.message || '',
                            status: 'pending',
                            challenge: {
                                title: request.challenge?.title || 'Başlık Yok',
                                game: request.challenge?.game || 'Oyun Belirtilmemiş',
                                platform: request.challenge?.platform || 'Platform Belirtilmemiş'
                            }
                        })),
    ...
    const loadActiveMatches = async () => {
        try {
            setLoading(true);
            const response = await challengeService.getMyActiveMatches(matchStatus !== 'all' ? matchStatus : undefined);
                    ...
                    <div className={styles.cardImage}>
                        <div className={styles.oyunAd}>
                            <img
                                className={styles.oyunAdChild}
                                alt={match.game}
                                src={match.gameImage || 'default-game.png'}
                            />
                            <div className={styles.nba2k25}>{match.game}</div>
                        </div>
                        <div className={styles.dakikaWrapper}>
                            <div className={styles.dakika}>⏱ {match.duration || '37 dakika'}</div>
                        </div>
                    </div>
                </div>
                <div className={styles.header}>
                    <div className={styles.oyuncu2Parent}>
                        <div className={styles.oyuncu2}>
                            ...
                            <img
                                ...
                                src={match.host.avatarUrl || 'default-avatar.png'}
                            />
                        </div>
                        <div className={styles.vs}>
                            <div className={styles.vs1}>VS</div>
                        ...
                        <div className={styles.oyuncu3}>
                            <img
                                ...
                                src={match.opponent.avatarUrl || 'default-avatar.png'}
                            />
                            <div className={styles.oyuncu1Info1}>
  	...
  	return (<>
    		<div className={styles.malarmGelenStekler}>
      			...
      			<div className={styles.container}>
        				...
        				<div className={styles.matchStatusFilter}>
                        <select 
                            value={matchStatus} 
                        ...
                        >
                            ...
                            <option value="pending">Bekleyen Maçlar</option>
                            <option value="completed">Tamamlanan Maçlar</option>
                            <option value="disputed">İtirazlı Maçlar</option>
                        </select>
                    </div>
