...
const Matches = () => {
    ...
    useEffect(() => {
        async function fetchUserChallenges() {
            try {
                ...
                // MeydanOkumalar.jsx'deki gibi filtreleri kullan
                const filters = {
                    page: 1,
                    limit: 10,
                    status: 'all', // 'active' yerine 'all' kullanarak tüm meydan okumaları al
                    game: '',
                    platform: '',
                    category: '',
                    difficulty: '',
                    search: '',
                    creator: true // Sadece kullanıcının kendi meydan okumalarını getir
                };
        ...
        try {
            ...
            if (selectedTab === 'gelen') {
                ...
                if (response.success) {
                    ...
                    // Bekleyen ve kabul edilen istekleri birleştir
                    const allRequests = [
    ...
    const handleAcceptRequest = async (requestId) => {
        try {
            setLoading(true);
            await challengeService.acceptRequest(requestId);
    ...
    const handleDeclineRequest = async (requestId) => {
        try {
            setLoading(true);
            await challengeService.declineRequest(requestId);
    ...
    const handleCancelRequest = async (requestId) => {
        try {
            setLoading(true);
            await challengeService.cancelRequest(requestId);
    ...
    // Tab değiştiğinde istekleri yükle
    useEffect(() => {
        loadRequests();
    }, [selectedTab]);
            ...
            <div key={request.id} className={styles.tableRow}>
                ...
                <div className={styles.cell}>
                    <div className={styles.timeRemaining}>
                        <span>⏱ {request.challenge?.duration || '0'} dakika</span>
                    </div>
                </div>
                    ...
                    <div className={styles.actions}>
                        {selectedTab === 'gelen' ? (
                            <>
                                ...
                                >
                                    <img src={declineIcone} alt="Reddet" />
                                </button>
                            </>
                        ) : (
