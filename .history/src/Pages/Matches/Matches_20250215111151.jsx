...
const MalarmGelenStekler = () => {
    ...
    useEffect(() => {
        async function fetchGelenIstekler() {
            try {
                setLoading(true);
                console.log('Seçili meydan okuma ID:', selectedChallenge);
                
                if (selectedChallenge) {
                    const requests = await challengeService.getChallengeRequests(selectedChallenge);
                    console.log('Gelen İstekler:', requests);
                    setGelenIstekler(requests);
                }
            } catch (error) {
                console.error('Gelen istekler yüklenirken hata:', error);
        ...
        if (selectedTab === 'gelen' && selectedChallenge) {
            console.log('fetchGelenIstekler çağrılıyor...');
            fetchGelenIstekler();
        }
