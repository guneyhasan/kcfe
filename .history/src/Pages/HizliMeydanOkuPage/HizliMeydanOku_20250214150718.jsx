...
const HizliMeydanOku = () => {
  ...
  const handleCreate = async () => {
    try {
      const challengeData = {
        title: `${selectedGame} ${selectedMode}`, // örn: "FC 25 Tekli Eleme"
        description: `${selectedGame} oyununda ${selectedMode} maçı`, // örn: "FC 25 oyununda Tekli Eleme maçı"
        difficulty: "medium",
        category: "fps",
        points: parseInt(selectedFee), // Katılım ücreti
        game: selectedGame,
        platform: selectedConsole,
        duration: 7, // Varsayılan 7 gün
        maxParticipants: selectedMode === 'Tekli Eleme' ? 2 : 4, // Mod'a göre katılımcı sayısı
        communicationLink: "https://discord.gg/xyz" // Varsayılan iletişim linki
      };

      const response = await challengeService.createChallenge(challengeData);
      
      if (response) {
        toast.success('Meydan okuma başarıyla oluşturuldu!');
        // Form'u temizle
        setSelectedGame('');
        setSelectedConsole('');
        setSelectedFee('');
      ...
      toast.error('Meydan okuma oluşturulurken bir hata oluştu');
    }
  };
