...
export const challengeService = {
    ...
    // Meydan okumaya gelen istekleri getir
    getChallengeRequests: async (challengeId) => {
        try {
            ...
            const response = await axios.get(
                `/api/challenges/${challengeId}/requests`,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
            ...
            if (error.response?.status === 403) {
                throw new Error('Bu meydan okumanın isteklerini görme yetkiniz yok');
            ...
            throw new Error(error.response?.data?.error?.message || 'İstekler alınamadı');
        }
    }
};
