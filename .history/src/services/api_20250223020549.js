...
export const challengeService = {
    ...
    // Giden istekleri getir
    getSentRequests: async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(
                `/api/challenges/requests/sent`,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
    ...
    getMyRequests: async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('/api/challenges/my-requests', {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    acceptRequest: async (requestId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post(`/api/challenges/requests/${requestId}/accept`, {}, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    declineRequest: async (requestId) => {
        try {
            const userData = localStorage.getItem('user');
            const token = JSON.parse(userData).access_token;
            const response = await axios.post(`/api/challenges/requests/${requestId}/decline`, {}, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
